...
bool check(int row, int col, int h, int w){
    for0(8){
        for0j(8){
            if(row+x[i] >=0 && row+x[i]<h && col+y[i]>=0 && col+y[i]<w){
                if(table[row+x[i]][col+y[i]] == 1)
                    return false;
            }
        }
    }
...
void printarr(int arr[][21], int h, int w){
    for0(h){
        for0j(w){
            cout << arr[i][j];
        }
...
int main() {
    ...
    while(t--){
        ...
        for1(w-1){
            if(check(h-1, w-1-i, h, w)){
                table[h-1][w-1-i] = 1;
                count++;
            }
        }
        for1(h-1){
            if(check(h-1-i, 0, h, w)){
                table[h-1-i][0] = 1;
                count++;
            }
        }
