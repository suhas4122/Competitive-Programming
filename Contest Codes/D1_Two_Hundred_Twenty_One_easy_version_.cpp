...
void solve(){
    ...
    for1(n){
        ...
        if(i&1) rod[i] = rod[i-1] + (-1)*rod[i];
        else rod[i] = rod[i-1] + rod[i];
    }
    ...
    for0(q){
        ...
        cin >> l >> r;
        l--, r--;
        ll sum = abs(rod[r] - (l != 0 ? rod[l-1] : 0));
        cout << sum%3 << "\n";
    }
...
void solve1(){
    ...
    for0(q){
        ...
        cin >> l >> r;
        l--, r--;
        ll ev = even[r] - (l != 0 ? even[l-1] : 0);
        ll od = odd[r] - (l != 0 ? odd[l-1] : 0);
        ll ans = abs(ev - od);
        if(ans == 0){
            cout << 0 << "\n";
        }else if(ans&1){
