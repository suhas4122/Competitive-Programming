...
int count(int z) {
    ...
    z = z1[s.size() - 1] - z;
    for (int i = 0; i <= z; i++) {
        int x1 = lower_bound(all(z1), i) - z1.begin();
        int x2 = lower_bound(all(z2), z - i) - z2.begin();
        int ones = (x1 > 0 ? o1[x1 - 1] : 0) + (x2 > 0 ? o2[x2 - 1] : 0);
        o = min(o, ones);
    }
    return o;
}
