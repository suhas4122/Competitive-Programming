...
int main() {
    ...
    while(t--){
        ...
        cin >> m;
        vpll dragons(m);
        for0(m) cin >> dragons[i].fi >> dragons[i].se;
        sort(all(heros));
        // pr(heros);
        ll sum = 0;
        for0(n) sum += heros[i];
        for0(m){
            auto it = lower_bound(all(heros), dragons[i].fi);
            ll coins;
            if(it == heros.end()){
                coins = dragons[i].fi - heros[n-1];
                coins += max(0LL, dragons[i].se - sum + heros[n-1]);
            }
            else if(*it == dragons[i].fi){
                coins = max(0LL, dragons[i].se - sum + *it);
            }else{
                ll coins1 = max(0LL, dragons[i].se - sum + *it);
                ll coins2 = LLONG_MAX;
                if(it != heros.begin()){
                    it--;
                    coins2 = max(0LL, dragons[i].se - sum + *it) +dragons[i].fi - *it;
                }
                coins = min(coins1, coins2);
            }
            cout << coins << "\n";
        }
    }
