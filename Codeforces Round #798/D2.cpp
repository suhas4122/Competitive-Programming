...
void solve() {
    ...
    vector<pt> pts = convex_hull(points);
    vll xpts, ypts;
    for (auto& point : pts) {
        xpts.push_back(point.x);
        ypts.push_back(point.y);
    }
    sort(all(xpts));
    sort(all(ypts));
    ll szp = sz(xpts);
    ll ansx = (accumulate(all(xpts), 0) + szp - 1) / szp;
    ll ansy = (accumulate(all(ypts), 0) + szp - 1) / szp;
    cout << ansx << " " << ansy << endl;
}
