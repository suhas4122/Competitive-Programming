...
void solve(){
    ll n, m;
    cin >> n >> m;
    sight.assign(n*m, {0, 0});
    priority.assign(n*m, 0);
    for0(n*m) cin >> sight[i].first, sight[i].se = i;
    for0(n*m) priority[i] = i%m;
    sort(all(sight), comp);
    // for(int i = 0; i<n*m; i++){
    //     ll start = i;
    //     while(sight[i+1].first == sight[i].first){
    //         i++;
    //     }
    //     ll last = i;
    //     if(start != last){
    //         sort(sight.begin()+start, sight.begin()+last+1, comp);
    //     }
    // }
    vll seat(n*m);
    for0(n*m){
        seat[sight[i].second] = i;
    }
    // pr(seat);
    vvll seated(n, vll(m, 0));
    ll sum = 0;
    for0(n*m){
        ll a = seat[i] / m;
        ll b = seat[i] % m;
