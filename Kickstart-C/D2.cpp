...
ll rec(ll i, ll j, ll k) {
    ...
    ll ans = 0;
    if (i < j && s[i] == s[j]) {
        ans = (ans + rec(i + 1, j - 1, k - 2)) % N;
    }
    ll temp = (rec(i + 1, j, k) + rec(i, j - 1, k)) % N;
    temp = (temp - (rec(i + 1, j - 1, k) % N) + N) % N;
    while (temp < 0) temp += N;
    temp %= N;
    ans = (ans + temp) % N;
    dp[i][j][k] = (ans % N);
...
signed main() {
    ...
    while (t--) {
        ...
        ll n;
        cin >> n >> s;
        dp.assign(mxn, vvll(mxn, vll(mxn, -1)));
        ll ans = 0;
        for (ll len = 0; len < n; len++) {
            ll temp = (((fac[n - len] * fac[len]) % N) * rec(0, n - 1, len)) % N;
            ans = (ans + temp) % N;
        }
        ans = ans * inv(fac[n], N);
        ans %= N;
