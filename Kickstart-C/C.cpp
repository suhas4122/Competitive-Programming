...
#define forc(l, r) for (long long i = (long long)(l); i <= (int)(r); ++i)  // closed interver rightom l to r r inclusive
...
void solve() {
    ...
    for0(n) {
        if (a[i].fi.se) {
            p[i + left[i]] = {time_fall[i], a[i + left[i]].se + 1};
        } else {
            p[i - right[i]] = {time_fall[i], a[i - right[i]].se + 1};
        }
    }
    sort(all(p));
    for (auto &x : p) {
