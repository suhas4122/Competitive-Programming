...
void solve() {
    ...
    vector<ll> deg(n, 0);
    set<pair<ll, ll>> ans;
    for (ll i = start, cnt = 0; cnt < n - 1; i = inc(i, n), cnt++) {
        g[i].insert(inc(i, n));
        g[inc(i, n)].insert(i);
        deg[i]++;
        deg[inc(i, n)]++;
        ans.insert({min(i, inc(i, n)), max(i, inc(i, n))});
    }
    ll j;
    for (ll i = start; i != (start - 1 + n) % n; i = inc(i, n)) {
        j = i;
        if ((deg[inc(i, n)] & 1) != a[inc(i, n)]) {
            g[i].erase(inc(i, n)), g[inc(i, n)].erase(i);
            ans.erase({min(i, inc(i, n)), max(i, inc(i, n))});
            deg[i]--, deg[inc(i, n)]--;
            j = inc(j, n);
            while ((deg[j] & 1) == a[j] && j != i) j = inc(j, n);
