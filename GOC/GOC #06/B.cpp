...
ll ncr(ll n, ll r) {
    return modM(fact[n], modM(invfact[r], invfact[n - r]));
}
...
void solve() {
    ...
    if (x > n || y > n) {
        if (abs(x - y) != 2) {
            ...
            cout << modM(ncr(2 * n - 2, n - 1), power(2, ((x + y - 2 * n) / 2))) << endl;
        }
    } else {
        if (abs(x - y) < 2) {
            cout << 0 << endl;
...
int main() {
    ...
    fact.resize(2e5 + 10);
    invfact.resize(2e5 + 10);

    fact[0] = fact[1] = invfact[0] = invfact[1] = 1;
    for (int i = 2; i <= sz(fact); i++) {
        fact[i] = modM(i, fact[i - 1]);
        invfact[i] = inv(fact[i], N);
    }
