...
void solve() {
    ...
    vector<vector<vector<ll>>> dp(2, vvll(n + 1, vll(k + 1, 0)));
    ...
    for(int i = 1; i <= n; i++) {
        for(int j = 1; j <= k; j++) {
            ...
            dp[1][i][j] = dp[1][i - 1][j];
            if(a[i - 1] % 2) {
                dp[0][i][j] = (dp[0][i][j] + dp[1][i - 1][j - 1] * a[i - 1]) % N;
                dp[1][i][j] = (dp[1][i][j] + dp[0][i - 1][j - 1] * a[i - 1]) % N;
            } else {
                dp[0][i][j] = (dp[0][i][j] + dp[0][i - 1][j - 1] * a[i - 1]) % N;
                dp[1][i][j] = (dp[1][i][j] + dp[1][i - 1][j - 1] * a[i - 1]) % N;
            }
        }
    }
    cout << dp[0][n][k] << endl;
