...
void pre() {
    vector<ll> primes(maxn + 100, 1);
    for (ll i = 2; i * i <= maxn + 10; i++) {
        if (primes[i] == 1) {
...
void solve() {
    ...
    // function<ll(ll, ll, ll)> rec = [&](ll u, ll par, ll type) -> ll {
    //     if (dp[u][type - 1] != -1) {
    //         return dp[u][type - 1];
    //     }
    //     ll ans;
    ...
    ll ans = rec(0, -1, 1);
    ans = modA(ans, rec(0, -1, 2));
    ans = modA(ans, rec(0, -1, 3));
