...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(mpmin.find(pre) != mpmin.end()) {
            minr = min(minr, sum - mpmin[pre]);
            mpmin[pre] = max(mpmin[pre], sum);
        } else {
            mpmin[pre] = sum;
        }

        if(mpmax.find(pre) != mpmax.end()) {
            maxr = max(minr, sum - mpmax[pre]);
            mpmax[pre] = min(mpmax[pre], sum);
        } else {
            mpmax[pre] = sum;
        }
    }    
