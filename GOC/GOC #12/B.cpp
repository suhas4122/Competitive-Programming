...
vll dijkstra(ll u, vector<vector<pair<ll, ll>>> &g) {
    vll dist(n + 1, 1e18);
    ...
    while (!pq.empty()) {
        ...
        if (vis[v]) continue;
        vis[v] = 1;
        for (auto& [neigh, dis] : g[v]) {
            if (dist[neigh] > dist[v] + dis) {
                dist[neigh] = dist[v] + dis;
...
void solve() {
    ...
    vll sd = dijkstra(s, g1);
    vll dd = dijkstra(d, g2);
    ll ans = 1e18;
    bool pos = true;
    for (int i = 1; i <= n; i++) {
        if (sd[i] == 1e18 || dd[i] == 1e18) {
            pos = false;
            break;
        }
        ans = min(ans, sd[i] + dd[i] + 1);
    }
    if (pos == false)
        cout << -1 << '\n';
    else
