...
void solve() {
    ...
    sort(all(cf), greater<pll>());
    sort(all(df), greater<pll>());
    ll ans1 = 0;
    int flag = 2;
    for (auto &x : cf) {
        if (x.se <= c) {
    ...
    multiset<ll> ms;
    sort(all(cf), comp);
    for (auto &x : cf) ms.insert(x.first);
    ll i = 0, j = cf.size() - 1;
    while (i < n) {
        if (j <= i) {
            break;
        }
        ms.erase(ms.find(cf[i].fi));
        while (cf[j].se + cf[i].se > c && j > i) {
            ms.erase(ms.find(cf[j].fi));
            j--;
        }
        if (j > i) {
            ans2 = max(ans2, cf[i].fi + *ms.rbegin());
        }
        i++;
    }
    ll ans3 = 0;
    ms.clear();
    sort(all(df), comp);
    for (auto &x : df) ms.insert(x.first);
    i = 0, j = df.size() - 1;
    while (i < n) {
        if (j <= i) {
            break;
        }
        ms.erase(ms.find(df[i].fi));
        while (df[j].se + df[i].se > d && j > i) {
            ms.erase(ms.find(df[j].fi));
            j--;
        }
        if (j > i) {
            ans3 = max(ans3, df[i].fi + *ms.rbegin());
        }
        i++;
    }
