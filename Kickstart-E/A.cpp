...
int main() {
    ...
    while(t--){
        ...
        //     string s1;
        //     set<pair<ll, char>> pq;
        //     vector <pair<char, ll>> vp (mp.begin(), mp.end());
        //     for(auto x: vp){
        //         // pr(x);
        ...
        //     for0(n){
        //         pr(pq);
        //         for(auto x1 = pq.rbegin(); x1 != pq.rend(); x1++){
        //             pair<ll, char> x = *x1;
        //             if(x.second != s[i]){
        //                 pq.erase(x);
        //                 s1 += x.second;
        ...
        if(mx > sz(s) / 2) {
            ...
            multiset<pair<int, char>> avail;
            for(char c : s) {
                avail.insert({0, c});
            }
            string t = s;
            for(i = 0; i < sz(a); i++) {
                int count = avail.count({0, a[i].second}), j;
                avail.erase({0, a[i].second});
                for(auto ind : pos[a[i].second - 'a']) {
                    assert(!avail.empty());
                    t[ind] = (*avail.begin()).second;
                    avail.erase(avail.begin());
                }
                for(j = 0; j < count; j++) {
                    avail.insert({1, a[i].se});
                }
            }
            cout << t << endl;
        }
