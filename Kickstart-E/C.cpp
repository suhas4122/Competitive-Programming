...
int main() {
    ...
    while(t--){
        ...
        vector<vector<pair<ll, ll>>> vp(n, vpll(m));
        for0(n){
            ll prev = -1;
            for0j(m){
                if(v[i][j] == -2){
        ...
        vector<vector<pair<ll, ll>>> vv(n, vpll(m));
        for0(m){
            ll prev = -1;
            for0j(n){
                if(v[j][i] == -2){
        ...
        //         int j = x.second;
        //         if(v[i][j] == -1){
        //             ll h = vp[i][j].first + vp[i][j].second - j;
        //             ll ve = vv[i][j].first + vv[i][j].second - i;
        //             if(v[i][h] != -1 && v[i][h] != -2){
        //                 counter++;
        //                 v[i][j] = v[i][h];
        //                 flag = true;
        ...
        while(!ele.empty()) {
            set<pll>temp;
            for(auto p1 = ele.begin(); p1 != ele.end(); p1++) {
                pll p = *p1;
                ll i = p.first;
                ll j = p.second;
                if(v[i][j] != -1 && v[i][j] != -2) {
                    int h = vp[i][j].first + vp[i][j].second - j;
                    ...
                    int vi = vv[i][j].first + vv[i][j].second - i;
