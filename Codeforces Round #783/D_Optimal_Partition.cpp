...
struct SegmentTree {
    ...
    int query_tree(int i, int l, int r, int lq, int rq) {
        if (l > rq || r < lq)
            return INT_MIN;
        if (l >= lq && rq >= r) {
            return tree[i];
            ...
            int mid = (l + r) / 2;
            return max(query_tree(2 * i, l, mid, lq, rq), 
    ...
    int query(int lq, int rq) { return query_tree(1, 0, n - 1, lq, rq); }
};
...
void solve() {
    ...
    SegmentTree st(n);
    st.arr.assign(n, INT_MIN);
    st.build();
    vll dp(n, 0);

    for(int i = 0; i < n; i++) {
        dp[i] = (i > 0 ? dp[i - 1] : 0) + (a[i] > 0 ? 1 : (a[i] < 0 ? -1 : 0));
        dp[i] = max(dp[i], 1LL * st.query(0, order[i]) + i + 1);
        if(pre[i] > 0) dp[i] = i + 1;
        st.update(order[i], dp[i] - (i + 1));
    }
