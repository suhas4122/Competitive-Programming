...
void solve() {
    ...
    vector<vector<int>> ra(n, vector<int>(n + 1, 0));
    for(int i = 0; i < n; i++) {
        vector<pair<ll, int>> dist;
        for (int j = 0; j <= n; j++) {
            ll d = 1LL * (s[j].fi - v[i].fi) * (s[j].fi - v[i].fi) + 1LL * (s[j].se - v[i].se) * (s[j].se - v[i].se);
            dist.pb({d, j});
        }
        sort(all(dist), [&](pair<ll, int> &x1, pair<ll, int> &x2) {
            if (x1.fi == x2.fi) return x1.se > x2.se;
            return x1.fi < x2.fi;
        });
        for (int j = 0; j <= n; j++) {
            ra[i][j] = dist[j].se;
    ...
    do {
        ...
        if (flag) {
            cout << "POSSIBLE" << endl;
            for(int i = 0; i < n; i++) {
                cout << perm[i] + 1 << " " << ans[perm[i]] + 1 << endl;
            }
            return;
        }
