...
void solve() {
    ll n, p;
    cin >> n >> p;
    vvll pressure(n, vll(p));
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < p; j++) {
            cin >> pressure[i][j];
        }
    }
    vvll dp(n, vll(2, 1e18));  // 0 - increasing, 1 - decreasing
    dp[0][0] = *max_element(all(pressure[0]));
    for (ll i = 1; i < n; i++) {
        ll prev_max = *max_element(all(pressure[i - 1]));
        ll prev_min = *min_element(all(pressure[i - 1]));
        ll curr_max = *max_element(all(pressure[i]));
        ll curr_min = *min_element(all(pressure[i]));
        dp[i][0] = dp[i-1][0] + abs(curr_min - prev_max) + abs(curr_max - curr_min);
        dp[i][0] = min(dp[i][0], dp[i-1][1] + abs(curr_min - prev_min) + abs(curr_max - curr_min));
    
        dp[i][1] = dp[i-1][0] + abs(curr_max - prev_max) + abs(curr_max - curr_min);
        dp[i][1] = min(dp[i][1], dp[i-1][1] + abs(curr_max - prev_min) + abs(curr_max - curr_min));
    }
