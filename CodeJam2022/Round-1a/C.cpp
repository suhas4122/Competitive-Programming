...
void solve() {
    ...
    for (int i = 0; i < e; i++) {
        ...
        vll temp(w, 0);
        vvll y(e, x[i]);
        for (int i1 = i + 1; i1 < e; i1++) {
            for (int j = 0; j < w; j++) {
                y[i1 - i][j] = min(y[i1 - i - 1][j], x[i1][j]);
            }
        }
        for (int j = e - i - 1; j >= 0; j--) {
            ...
            for (int k = 0; k < w; k++) {
                if (temp[k] < min_x[k]) {
                    int p = min_x[k] - temp[k];
                    // pr(p, k);
                    for (int l = 0; l < p; l++) arr.pb(k);
                }
            }
            temp = min_x;
        }
        ...
        for(int i1 = 0; i1 < old.size(); i1++) {
            b[old[i1]]++;
        }
        for(auto it = a.begin(); it != a.end(); it++) {
            if(b.find(it->fi) != b.end()) {
                counter += min(it->se, b[it->fi]);
            }
        }
        counter = sz(arr) + sz(old) - 2*counter;
        pr(counter);
        pr(arr);
        old = arr;
    }
