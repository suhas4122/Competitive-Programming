...
void solve() {
    ll n, m;
    cin >> n >> m;
    vvll dp(n + 1, vll((1 << m), -1));

    function<ll(ll, ll, ll, ll)> rec = [&](ll x, ll y, ll vis, ll pvis) -> ll {
        if (x > n) {
            if (pvis == (1 << m) - 1)
                return 1;
            return 0;
        ...
        if (y >= m) {
            return rec(x + 1, 0, 0, vis);
        }
        if (dp[x][pvis] != -1 && vis == 0) {
            return dp[x][pvis];
        }
        if ((pvis & (1 << y)) == 0) {
            return dp[x][pvis] = rec(x, y + 1, vis | (1 << y), pvis);
        } else {
            ll ans = 0;
            ans += rec(x, y + 1, vis, pvis);
            if (y < m - 1 && (pvis & (1 << (y + 1))) != 0) {
                ans += rec(x, y + 2, vis | (1 << y) | (1 << (y + 1)), pvis);
            }
            return dp[x][pvis] = ans;
        }
    };
