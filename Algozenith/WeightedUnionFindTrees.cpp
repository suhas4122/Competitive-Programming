...
struct DSU {
    ...
    pll find(ll x) {
        ...
        pll ret = find(arr[x].fi);
        return arr[x] = {ret.fi, ret.se + arr[x].se};
    }

    void unite(ll x, ll y, ll dist) {
        pll x1 = find(x);
        pll y1 = find(y);
        if (x1.fi == y1.fi) return;
        if (rank[x1.fi] > rank[y1.fi]){
            swap(x1, y1);
            dist *= -1;
        }
        arr[x1.fi] = {y1.fi, dist - x1.se + y1.se};
        rank[y1.fi] += rank[x1.fi];
        size--;
    }
};
