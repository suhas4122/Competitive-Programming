...
void solve() {
    ...
    function<ll(ll, ll, ll, ll, ll, ll)> rec = [&](ll idx, ll lo, ll hi, ll num, ll mask, ll last) -> ll {
        if (idx == n) {
            ll cnt = 0;
            for (ll i = 0; i < 9; i++) {
                if (mask & (1 << i)) {
        ...
        ll& ans = dp[idx][lo][hi][num][mask][last];
        if (ans != -1) return ans;
        ans = 0;
        ll nlo = (lo ? l[idx] - '0' : 0);
        ll nhi = (hi ? r[idx] - '0' : 9);
        for (ll i = nlo; i <= nhi; i++) {
            ll nmask = (i > 0 ? (mask | (1 << (i - 1))) : mask);
            ans += rec(idx + 1, lo & (i == nlo), hi & (i == nhi), (num * 10 + i) % 504, nmask, (i == 0) || (i == 5));
        }
        return ans;
    };
