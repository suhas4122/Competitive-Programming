...
ll cross_product(Point c, Point a, Point b)
{
...
// bool isLeft(Point c, Point , Point c){
...
{  
    ...
    if(A1 == 0) {
        ...
        for(int i = 0; i < n; i++) {
            ll v = cross_product(arr[i], data[2], data[0]);
            ll w = cross_product(data[1], data[2], data[0]);
            if(v < 0 && w > 0 || v > 0 && w < 0) {
                vector<Point> temp = {data[1], data[2], arr[i], data[0]};
                long double pt = perimeter(temp);
                // pr(pt);
                ans = min(ans, pt);
            }
