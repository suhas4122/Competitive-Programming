...
int main() {
    ...
    while(t--){
        ...
        cin >> d >> n >> k;
        vpll start, end;
        map<ll, multiset<ll>>startm, endm;
        vpll array;
        for0(n){
            ll a, b, c;
            cin >> a >> b >> c;
            array.push_back({b, a});
            array.push_back({c+1, -1*a});
            startm[b].insert(a);
            endm[c+1].insert(a);
        }
        ll ans = 0, sum = 0;
        multiset<ll> temp1, temp2;
        sort(all(array), comp);
        ll j = 0;
        // pr(array);
        for1(d){
            while(array[j].fi == i){
                if(array[j].se < 0){
                    array[j].se *= -1;
                    if(temp2.find(array[j].se) == temp2.end()) {
                        // pr(array[j].se);
                        temp1.erase(temp1.find(array[j].se));
                        sum -= array[j].se;
                        if(sz(temp2)) {
                            ll x = *temp2.rbegin();
                            temp1.insert(x);
                        ...
                        temp2.erase(temp2.find(array[j].se));
                    }
                }else{
                    if(sz(temp1) >= k) {
                        if(array[j].se > *temp1.begin()) {
                            ll x = *temp1.begin();
                            temp1.erase(temp1.begin());
                            temp1.insert(array[j].se);
                            temp2.insert(x);
                            sum += array[j].se - x;
                            ans = max(ans, sum);
                        }
                        else {
                            temp2.insert(array[j].se);
                        }
                    }
                    else {
                        temp1.insert(array[j].se);
