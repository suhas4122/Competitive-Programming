...
ll sum(ll v, ll tl, ll tr, ll l, ll r) {
    ...
    ll tm = (tl + tr) / 2;
    return modA(sum(v*2, tl, tm, l, min(r, tm)), sum(v*2+1, tm+1, tr, max(l, tm+1), r));
}
...
void solve() {
    ...
    for0(n) {
        ll curr = sum(1, 0, MAXN-1, 0, a[i]);
        curr = modA(curr, 1LL);
        update(1, 0, MAXN-1, a[i], curr);
    }
    cout << modA(sum(1, 0, MAXN-1, 0, MAXN-1), 1) << endl;
}
