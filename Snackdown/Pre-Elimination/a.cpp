...
void solve() {
    ...
    for(ll i = 0; i < n; i++) {
        for(ll j = i + 2; j < n; j++) {
            ll k = (a[i] + a[j]) / 2;
            ll l1 = lower_bound(all(a), k) - a.begin();
            ll l2 = upper_bound(all(a), k) - a.begin();
            ll mini = 0;
            for(ll l = l1 - 1; l <= l1 + 1; l++){
                if(l > i && l < j && ((a[i] - a[l]) * (a[l] - a[j])) > mini){
                    mini = (a[i] - a[l]) * (a[l] - a[j]);
                }
            }
            for(ll l = l2 - 1; l <= l2 + 1; l++){
                if(l > i && l < j && ((a[i] - a[l]) * (a[l] - a[j])) > mini){
                    mini = (a[i] - a[l]) * (a[l] - a[j]);
                }
            }
