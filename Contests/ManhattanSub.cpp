...
bool check3(int arr[], int i){
    return check(arr[i], arr[i+1], arr[i+2]);
}

bool check4(int arr[], int i){
    bool ans1 = check(arr[i], arr[i+1], arr[i+2]);
    bool ans2 = check(arr[i], arr[i+1], arr[i+3]);
    bool ans3 = check(arr[i], arr[i+2], arr[i+3]);
    bool ans4 = check(arr[i+1], arr[i+2], arr[i+3]);
...
int main() {
    ...
    while(t--){
        ...
        for0(n-1){
            ...
            if(i <= n-2)
                count++;
            if(i <= n-3 && check3(arr, i))
                count ++;
            if(i <= n-4 && check4(arr, i))
                count++;
            ans += count;
        }
        cout << ans + 1 << "\n";
