...
ll n, MOD;
const ll lim = 1005;
ll binpow(ll b, ll p){ ll ans = 1; while(p > 0){if(p & 1) ans = (ans * b); b = (b * b); p >>= 1;}return ans;}
...
vll solve()
{
    ...
    vector<ll> temp[5];
    for(ll pow = 1; pow <= 4; pow++) {
        for(i = 0; i < n; i++) {
            ll num = binpow(arr[i], pow) - binpow(arr[i] % MOD, pow);
            ll cnt = 0;
            while(num % MOD == 0 && num) {
                num /= MOD;
                cnt++;
            }
            temp[pow].push_back(cnt);
        }
        assert(temp[pow].size() == n);
    }
    ...
    FenwickTree s1(temp[1]), s2(temp[2]), s3(temp[3]), s4(temp[4]);
    ...
    while(q--) {
        ...
        if(ch == 1) {
            ...
            arr[pos] = val;
            for(ll pow = 1; pow <= 4; pow++) {
                ll num = binpow(arr[pos], pow) - binpow(arr[pos] % MOD, pow);
                ll cnt = 0;
                while(num % MOD == 0 && num) {
                    num /= MOD;
                    cnt++;
                ...
                if(pow == 1) {
                    s1.add(pos, cnt - b[1][pos]);
                    b[1][pos] = cnt;
                }
                else if(pow == 2) {
                    s2.add(pos, cnt - b[2][pos]);
                    b[2][pos] = cnt;
                }
                else if(pow == 3) {
                    s3.add(pos, cnt - b[3][pos]);
                    b[3][pos] = cnt;
                }
                else if(pow == 4) {
                    s4.add(pos, cnt - b[4][pos]);
                    b[4][pos] = cnt;
                }
            }
        }
