...
void solve() {
    ...
    for0(k){
        ll a, b, c, d, e;
        cin >> a >> b >> c >> d >> e;
        ladders.pb({a, b, c, d, e});
    }
    sort(all(ladders));
    map<pll, ll>dp;
    dp[{1,1}] = 0;
    vector<set<ll>> points(n+1);
    points[1].insert(1);
    for0(k){
        ll a, b, c, d, e;
        a = ladders[i][0];
        ...
        e = ladders[i][4];
        for(auto x: points[a]){
            if(dp.find({c, d}) == dp.end()) dp[{c, d}] = LLONG_MIN;
            dp[{c, d}] = max(dp[{c, d}], dp[{a, x}] + e - fl[a]*abs(b - x));
            points[c].insert(d);
        }
    }
    ll ma = LLONG_MIN;
    for(auto x: points[n])
        ma = max(ma, dp[{n, x}] - fl[n]*abs(x - m));
    if(ma != LLONG_MIN)
        cout << -1*ma << "\n";
    else    
        cout << "NO ESCAPE\n";
