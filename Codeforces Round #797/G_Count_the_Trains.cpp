...
void solve() {
    ...
    vector<int> a(n);
    for (auto &x : a) cin >> x;
    ordered_set<pair<int, int>> ranges;
    ...
    while (m--) {
        ...
        k--;
        int nval = a[k] - d;
        auto it = ranges.upper_bound({k, -1e9});
        it--;
        auto [i, j] = *it;
        int ind = ranges.order_of_key(*it);
        auto valit = vals.find_by_order(ind);
        int val = *valit;
        if (-val <= nval) {
            cout << sz(ranges) << " ";
            continue;
        }
        auto lit = vals.upper_bound(-nval);
        lit--;
        int valnind = vals.order_of_key(*lit);
        auto [ri, rj] = *ranges.find_by_order(valnind);
        for (int iter = valnind; iter >= ind + 1; iter--) {
            ranges.erase(ranges.find_by_order(iter));
            vals.erase(vals.find_by_order(iter));
        }
        ranges.erase(ranges.find_by_order(ind));
        if (k > i) {
            ranges.insert({i, k - 1});
        } else {
            vals.erase(valit);
