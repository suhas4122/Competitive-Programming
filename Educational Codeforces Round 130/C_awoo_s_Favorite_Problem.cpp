...
void solve() {
    ...
    vector<int> pref[3];
    for (int i = 0; i < 3; i++) {
        pref[i] = vector<int>(n + 1, 0);
    }
    for (char c = 'a'; c <= 'c'; c++) {
        for (int i = 0; i < n; i++) {
            pref[c - 'a'][i + 1] = pref[c - 'a'][i + 1] + (s[i] == c);
        }
    }
    bool f = 1;
    set<int> bs, cs, as;
    ...
    for (int i = 0; i < n; i++) {
        while (!bs.empty() && *bs.begin() < i) {
            bs.erase(bs.begin());
        }
        while (!cs.empty() && *cs.begin() < i) {
            cs.erase(cs.begin());
        }
        while (!as.empty() && *as.begin() < i) {
            as.erase(as.begin());
        }
        if (t[i] == s[i]) {
            continue;
        ...
        if (t[i] == 'b' && s[i] == 'a') {
            auto itb = bs.lower_bound(i);
            if (itb == bs.end()) {
                f = 0;
                break;
            }
            int j = *itb;
            auto itc = cs.lower_bound(i);
            if (itc == cs.end() || *itc > j) {
                swap(s[i], s[j]);
                bs.erase(j);
            } else {
