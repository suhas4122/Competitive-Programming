...
void solve() {
    ...
    for(int i = 0; i < n; i++) {
        for(int j = i + 1; j < n; j++) {
            ll dist = abs(a[i].fi - a[j].fi) + abs(a[i].se - a[j].se);
            mp[dist].insert(i); 
            mp[dist].insert(j); 
        }
    }
    ...
    for(auto &pp: mp) {
        ...
        for(int i = 0; i < m; i++) {
            ...
            for(int j = i + 1; j < m; j++) {
                int dist2 = abs(a[ele[i]].fi - a[ele[j]].fi) + abs(a[ele[i]].se - a[ele[j]].se);
                if(dist2 != dist) {
                    flag = false;
                }
            }
