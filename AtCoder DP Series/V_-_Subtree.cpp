...
void solve() {
    ...
    function<void(ll, ll)> dfs_down = [&] (ll u, ll par) -> void {
        down[u] = 1; 
        for(auto &v: g[u]) {
            if(v != par) {
                dfs_down(v, u);
                down[u] = modM(down[u], down[v]);
            }
        }
        down[u]++;
    };
    ...
    function<void(ll, ll)> dfs_up = [&] (ll u, ll par) -> void {
        ll sum = up[u];
        for(auto &v: g[u]) {
            if(v != par) {
                up[v] = modM(up[v], sum);
    ...
    for(int i = 0; i < n; i++) {
        ll ans = modM(modS(down[i], 1), up[i]);
        cout << ans << endl;
    }
}
...
// ll mod(ll n) { return (n % mod + N) % N; }
ll modM(ll n, ll m) { return ((n % mod * m % mod) + mod) % mod; }
ll modA(ll n, ll m) { return ((n % mod + m % mod) + mod) % mod; }
