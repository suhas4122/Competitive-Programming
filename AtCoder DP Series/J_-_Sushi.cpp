...
void solve() {
    ...
    function<ld(ll, ll, ll)> rec = [&](ll x, ll y, ll z) -> ld {
        if (x == 1 && y == 0 && z == 0) {
            return 1;
        }
        if(dp[x][y][z] > 0) {
        ...
        if (x) {
            ld prob = ((ld) x) / (x + y + z);
            ld invprob = ((ld)(x + y + z) / (x + y + z - 1));
            ans += prob * (1 + invprob * rec(x - 1, y, z));
        } 
        if (y) {
            ld prob = ((ld) y) / (x + y + z);
            ld invprob = 1;
            ans += prob * (1 + invprob * rec(x + 1, y - 1, z));
        } 
        if (z) {
            ld prob = ((ld) z) / (x + y + z);
            ld invprob = 1;
            ans += prob * (1 + invprob * rec(x, y + 1, z - 1));
        } 
        return dp[x][y][z] = ans;
    };
